...
OtiUCL <- function(U){#Optimization function used to find the UCL and LCL
  ...
  #Calculating the ARL through absorbing states
...
par_optim <- optimize(OtiUCL,c(0,1),tol = .Machine$double.eps)
Ua <- par_optim$minimum
...
#X-bar control chart
pxi=pnorm(LCLxb,u1,s0/(n^0.5))
pxs=1-pnorm(UCLxb,u1,s0/(n^0.5))
pxc=1-pxs-pxi

################################################################################
#Now that we have the probabilities for this specific case (u1), we solve it 
#through a Markov Chain again
#Markov chain
size<- 5 #Size of the markov chain
MarkovChain<- matrix(0,nrow=size,ncol=size,byrow=TRUE)
MarkovChain[1,]<- c(pxc,pxs,pxi,0,0) 
